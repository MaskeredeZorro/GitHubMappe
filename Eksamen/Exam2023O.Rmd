...
It can be seen in the output that the delimiter is a comma. And we load the dataset using `read_csv`
...
The number of rows are 3106. There are
...
res1 <- dat %>% 
  ...
  count(job_id) %>% 
  summarize(min = min(n), avg=mean(n), max=max(n)) %>% 
  print()
...
res1 <- dat %>% 
  group_by(item_id) %>% 
  summarise(items = sum(item_quantity), item_name= first(item_name)) %>% 
  arrange(desc(items)) %>% 
  print()
```
