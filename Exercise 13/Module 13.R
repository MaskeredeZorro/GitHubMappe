...
#Do you think it's more likely to find bookings with children or babies in city 
  ...
  #It seems more bookings at City Hotel than Resort Hotel
  ...
  What is happening in the following chunk?
    ...
    ```{r}
  hotels %>%
    mutate(little_ones = children + babies) %>% #Her sætter vi to variabler/kolonner sammen til en
    count(hotel, little_ones) %>% #Vi tæller hvor mange children and babies der er på hotellerne
    mutate(prop = n / sum(n)) # Laver ny kolonne som er lig med antal af children og babies / med antal obs. i alt
  ```
  ...
  ### count is simply short for `group_by` for grouped operations
