...
mean_dk <- gapminder %>% 
 ...
 gapminder %>% 
   mutate(relative=gdpPercap/mean_dk) %>% 
...
gapminder %>% 
  group_by(continent) %>% 
  summarize(n = n(), n_countries = n_distinct(country), avg_lifeExp = mean(lifeExp))
```
...
gapminder %>%
  ...
  arrange(year, .by_group = TRUE) %>%   # make sure that data is sorted correct (so first is 1952)
  mutate(lifeExp_gain = lifeExp-first(lifeExp)) %>%        # have a look at the ?first function
  filter(year < 1963)                   # just for nice printing

#increased for Albandia, for instance (6), by 9,59 years since 1962.
``` 
